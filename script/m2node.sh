...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/clavin-dev/m2node/main/script/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/clavin-dev/m2node/main/script/install.sh) $version
    if [[ $? == 0 ]]; then
...
uninstall() {
    ...
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        rm /etc/systemd/system/m2node.service -f
        systemctl daemon-reload
        systemctl reset-failed
    fi
    rm /etc/m2node/ -rf
    rm /usr/local/m2node/ -rf

    echo ""
    echo -e "卸载成功，如果你想删除此脚本，则退出脚本后运行 ${green}rm /usr/bin/m2node -f${plain} 进行删除"
    echo ""
...
show_log() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        journalctl -u m2node.service -e --no-pager -f
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
update_shell() {
    wget -O /usr/bin/m2node -N --no-check-certificate https://raw.githubusercontent.com/clavin-dev/m2node/main/script/m2node.sh
    if [[ $? != 0 ]]; then
        echo ""
        echo -e "${red}下载脚本失败，请检查本机能否连接 Github${plain}"
        before_show_menu
    else
        chmod +x /usr/bin/m2node
...
check_status() {
    if [[ ! -f /usr/local/m2node/v2node ]]; then
        return 2
    fi
    if [[ x"${release}" == x"alpine" ]]; then
...
show_v2node_version() {
    echo -n "v2node 版本："
    /usr/local/m2node/v2node version
    echo ""
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
generate_v2node_config() {
        ...
        mkdir -p /etc/m2node >/dev/null 2>&1
        cat > /etc/m2node/config.json <<EOF
{
    "Log": {
        "Level": "warning",
        "Output": "",
...
--- https://github.com/clavin-dev/m2node ---
  ${green}0.${plain} 修改配置
————————————————
